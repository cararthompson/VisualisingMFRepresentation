...
wordsSentiments %>%
  ...
  left_join(wordsSentiments %>%
            ...
            by = "sentiment") %>%
  rename(`Predominantly Female sentences` = Female.x,
         `Neutral sentences` = Neutral.x,
         `Predominantly Male sentences` = Male.x) %>%
  arrange(desc(Neutral.y)) %>%
  select(sentiment, `Predominantly Female sentences`,
         `Neutral sentences`,
         `Predominantly Male sentences`) %>%
  gt(groupname_col = NULL, rowname_col = "sentiment") %>%
  fmt_missing(columns = vars(`Predominantly Female sentences`, 
                             `Predominantly Male sentences`, 
                             `Neutral sentences`),
              missing_text = "-") %>%
  tab_style(
    style = cell_fill(color = fColour, alpha = 0.1),
    locations = cells_body(
      columns = vars(`Predominantly Female sentences`))) %>%
  tab_style(
    style = cell_fill(color = mColour, alpha = 0.1),
    locations = cells_body(
      columns = vars(`Predominantly Male sentences`))) %>%
  tab_style(style = "vertical-align:top",
            locations = cells_body(columns = vars(sentiment, 
                                                  `Predominantly Female sentences`, 
                                                  `Predominantly Male sentences`, 
                                                  `Neutral sentences`))) %>%  
  # This doesn't work
  tab_style(
